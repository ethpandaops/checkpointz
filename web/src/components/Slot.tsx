...
export default function Slot(props: { slot: number }) {
  ...
  const { data, isLoading } = useQuery<APIBeaconSlotBlock, Error, APIBeaconSlotBlock, [string, { slot: number }]>(['beacon_slot', { slot: props.slot }], async ({ queryKey }) => {
    const [, { slot }] = queryKey;
  ...
  if (!block?.message) return (
    <div className="flex justify-center font-bold">
      <ExclaimationTriangleIcon className="h-10 w-10 text-yellow-400 pr-1 " aria-hidden="true" /><span className="text-2xl pt-1">Something went wrong</span>
    </div>
  );
  return (
      ...
      <div className="border-t border-gray-200 px-4 py-5 sm:p-0">
        <dl className="sm:divide-y sm:divide-gray-200">
          <div className="py-4 sm:py-5 sm:grid sm:grid-cols-5 sm:gap-4 sm:px-6">
            ...
            <dd className="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-4">{data?.data?.epoch}</dd>
          </div>
          <div className="py-4 sm:py-5 sm:grid sm:grid-cols-5 sm:gap-4 sm:px-6">
            <dt className="text-sm font-medium text-gray-500">Slot</dt>
            <dd className="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-4">{block.message.slot}</dd>
          </div>
          <div className="py-4 sm:py-5 sm:grid sm:grid-cols-5 sm:gap-4 sm:px-6">
            <dt className="text-sm font-medium text-gray-500">Proposer</dt>
            <dd className="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-4">{block.message.proposer_index}</dd>
          </div>
          <div className="py-4 sm:py-5 sm:grid sm:grid-cols-5 sm:gap-4 sm:px-6">
            ...
            <dd className="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-4">
              {block.message.body?.execution_payload?.block_hash && (<>
                ...
                <span className="hidden lg:table-cell font-mono">{block.message.body.execution_payload.block_hash}<CopyToClipboard text={block.message.body.execution_payload.block_hash} /></span>
              </>)}
            </dd>
          </div>
          <div className="py-4 sm:py-5 sm:grid sm:grid-cols-5 sm:gap-4 sm:px-6">
            ...
            <dd className="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-4">
              {block.message.parent_root && (<>
                ...
                <span className="hidden lg:table-cell font-mono">{block.message.parent_root}<CopyToClipboard text={block.message.parent_root} /></span>
              </>)}
            </dd>
          </div>
          <div className="py-4 sm:py-5 sm:grid sm:grid-cols-5 sm:gap-4 sm:px-6">
            ...
            <dd className="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-4">
              {block.message.state_root && (<>
                ...
                <span className="hidden lg:table-cell font-mono">{block.message.state_root}<CopyToClipboard text={block.message.state_root} /></span>
              </>)}
            </dd>
          </div>
          <div className="py-4 sm:py-5 sm:grid sm:grid-cols-5 sm:gap-4 sm:px-6">
            <dt className="text-sm font-medium text-gray-500">Signature</dt>
            <dd className="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-4 break-all font-mono">{block.signature}</dd>
          </div>
          <div className="py-4 sm:py-5 sm:grid sm:grid-cols-5 sm:gap-4 sm:px-6">
            <dt className="text-sm font-medium text-gray-500">Randao Reveal</dt>
            <dd className="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-4 break-all font-mono">{block.message.body?.randao_reveal}</dd>
          </div>
          <div className="py-4 sm:py-5 sm:grid sm:grid-cols-5 sm:gap-4 sm:px-6">
            ...
            <dd className="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-4 break-all font-mono flex items-center justify-between ">
              ...
              <span className="whitespace-nowrap pl-1">
                <button
                  type="button"
                  className={clsx(
                    showFullHash ? 'bg-fuchsia-400 text-fuchsia-100 hover:bg-fuchsia-500' : 'shadow-inner bg-fuchsia-100 text-fuchsia-600 hover:bg-fuchsia-200',
