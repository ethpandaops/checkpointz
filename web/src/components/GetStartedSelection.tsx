...
const clients: ConsensusClient[] = [
        ...
        <div className="flex">
          ...
          <div className="ml-3">
            ...
            <div className="mt-2 text-sm text-yellow-700">
              <p>
                Refer to your client's documentation for more information on how to checkpoint sync.
              </p>
            </div>
          </div>
        </div>
  ...
  },
  {
    ...
    logCheck: (publicURL: string) => (
      <div className="bg-gray-100 p-5 rounded-lg grid">
        <pre className="overflow-x-auto">{`INFO Starting checkpoint sync                remote_url: ${publicURL}, service: beacon
...
`}
  ...
  },
  {
    name: 'Lodestar',
    image: LodestarImage,
    description: 'Lodestar is a TypeScript implementation of the Ethereum Consensus specification developed by ChainSafe Systems.',
    defaultPort: 9596,
    commandLine: (publicURL: string) => (
      <div className="bg-gray-100 p-5 rounded-lg grid">
        <pre className="overflow-x-auto">{`--weakSubjectivitySyncLatest=true
--weakSubjectivityServerUrl=${publicURL}`}
    ...
    logCheck: (publicURL: string) => (
      <div className="bg-gray-100 p-5 rounded-lg grid">
        <pre className="overflow-x-auto">{`info: Fetching weak subjectivity state weakSubjectivityServerUrl=${publicURL}
info: Download completed`}
  ...
  },
  {
    ...
    description: 'Nimbus is an extremely efficient Ethereum consensus layer client implementation developed by Status Research & Development.',
    defaultPort: 5052,
    commandLine: (publicURL: string) => (
      <div>
        Requires the <span className="font-mono bg-gray-100 p-1">trustedNodeSync</span> command to be run before the beacon is launched. Read more here: <a className="underline" href="https://nimbus.guide/trusted-node-sync.html">https://nimbus.guide/trusted-node-sync.html</a>
      </div>
    ),
    logCheck: (publicURL: string) => (
      <div className="bg-gray-100 p-5 rounded-lg grid">
        <pre className="overflow-x-auto">{`Starting trusted node sync                 databaseDir=/data/consensus/db restUrl=${publicURL} blockId=finalized backfill=false reindex=false
...
Database initialized, historical blocks will be backfilled when starting the node`}
  ...
  },
  {
    ...
    description: 'Prysm is an Ethereum proof-of-stake client written in Go developed by Prysmatic Labs.',
    defaultPort: 3500,
    commandLine: (publicURL: string) => (
      <div className="bg-gray-100 p-5 rounded-lg grid">
        <pre className="overflow-x-auto">{`--checkpoint-sync-url=${publicURL}
--genesis-beacon-api-url=${publicURL}`}
    ...
    logCheck: (publicURL: string) => (
      <div className="bg-gray-100 p-5 rounded-lg grid">
        <pre className="overflow-x-auto">{`level=info msg="requesting ${publicURL}/eth/v2/debug/beacon/states/genesis"
level=info msg="requesting ${publicURL}/eth/v2/debug/beacon/states/finalized"`}
  ...
  },
  {
    ...
    logCheck: (publicURL: string) => (
      <div className="bg-gray-100 p-5 rounded-lg grid">
        <pre className="overflow-x-auto">{`INFO  - Loading initial state from ${publicURL}
...
export default function GetStartedSelection(props: { onChange?: (client?: ConsensusClient) => void }) {
  ...
  return (
    <>
      <h1 className="text-2xl font-bold m-4 text-gray-800">Which Ethereum consensus client are you using?</h1>
                ...
                <>
                  <span className="flex items-center">
                    <span className="flex flex-col text-sm">
                      <RadioGroup.Label as="span" className="font-medium text-gray-900 flex items-center">
                        {client.image && (<img
                          src={client.image}
                          alt={client.name}
                          className={clsx(
